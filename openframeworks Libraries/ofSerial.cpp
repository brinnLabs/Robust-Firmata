...
vector <string> ofSerial::getDeviceFriendlyNames(){
	vector<string> temp;
	for (int i = 0; i < nPorts; i++){
		temp.push_back(portNamesFriendly[i]);
	}
	return temp;
}
#endif
...
bool ofSerial::setup(string portName, int baud){
...
#if defined FLOW_CONTROL_SOFTWARE
		cfg.dcb.fOutxCtsFlow = false;
		cfg.dcb.fRtsControl = RTS_CONTROL_DISABLE;
		cfg.dcb.fOutX = true;
		cfg.dcb.fInX = true;
#elif defined FLOW_CONTROL_HARDWARE
		cfg.dcb.fOutxCtsFlow = true;
		cfg.dcb.fRtsControl = RTS_CONTROL_TOGGLE;
		cfg.dcb.fOutX = false;
		cfg.dcb.fInX = false;
	...
	SetCommMask(hComm, EV_BREAK | EV_CTS | EV_DSR | EV_RLSD | EV_ERR | EV_RING | EV_RXCHAR | EV_RXFLAG | EV_TXEMPTY);
...
//----------------------------------------------------------------
int ofSerial::writeByte(unsigned char singleByte){
...
bool ofSerial::isAvailable(){
	...
	bool isAttached = GetCommState(hComm, &dcb);
	//cout << GetCommModemStatus(hComm, &dwEvtMask) << ", " << dwEvtMask << endl;
	return isAttached;
}

void ofSerial::printSettings(){
	DCB dcb;
	DWORD dwEvtMask;
	...
	ofLogNotice("Serial") << "fOutxCTSFlow: " << dcb.fOutxCtsFlow;
	ofLogNotice("Serial") << "fOutxDSRFlow: " << dcb.fOutxDsrFlow;
	ofLogNotice("Serial") << "fDSRSensitivity: " << dcb.fDsrSensitivity;
	ofLogNotice("Serial") << "fTXContinueOnXoff: " << dcb.fTXContinueOnXoff;
	ofLogNotice("Serial") << "fOutX: " << dcb.fOutX;
	ofLogNotice("Serial") << "fInX: " << dcb.fInX;
	ofLogNotice("Serial") << "fDtrControl: " << dcb.fDtrControl;
	ofLogNotice("Serial") << "fRtsControl: " << dcb.fRtsControl;

}

void ofSerial::printState(){
	DWORD dwEvtMask;
	/*WaitCommEvent(hComm, &dwEvtMask, NULL);*/


	if (GetCommModemStatus(hComm, &dwEvtMask)){
		if (dwEvtMask & MS_CTS_ON != Cts)
		{
			Cts = !Cts;
			cout << " CTS Changed" << endl;
		}

		if (dwEvtMask & MS_DSR_ON != Dsr)
		{
			Dsr = !Dsr;
			cout << "DSR Changed" << endl;
		}

		if (dwEvtMask & MS_RING_ON != Ring)
		{
			Ring = !Ring;
			cout << "RING Changed" << endl;
		}

		if (dwEvtMask & MS_RLSD_ON != Rlsd)
		{
			Rlsd = !Rlsd;
			cout << "RLSD Changed" << endl;
		}
	}
	else {
		ofLogError("ofSerial") << GetLastError();
	}
}

void ofSerial::setRTS(bool level)
{
	if (bInited){
		if (level) {
...
void ofSerial::setDTR(bool level)
{
	if (bInited){
		if (level) {
...
bool ofSerial::waitForChange()
{
	if (bInited){
		DWORD dwCommEvent;

		if (!SetCommMask(hComm, EV_CTS | EV_DSR | EV_RING | EV_RLSD)) {
...
bool ofSerial::getCTS()
{
	if (bInited){
		DWORD dwModemStatus;
		if (!GetCommModemStatus(hComm, &dwModemStatus)) {
			ofLogError("ofSerial") << "Error getting the status of the CTS line.";
		}
...
bool ofSerial::getDSR()
{
	if (bInited){
		DWORD dwModemStatus;
		if (!GetCommModemStatus(hComm, &dwModemStatus)) {
			ofLogError("ofSerial") << "Error getting the status of the DSR line.";
		}
...
bool ofSerial::getRI()
{
	if (bInited){
		DWORD dwModemStatus;
		if (!GetCommModemStatus(hComm, &dwModemStatus)) {
			ofLogError("ofSerial") << "Error getting the status of the RI line.";
		}
...
bool ofSerial::getRLSD()
{
	if (bInited){
		DWORD dwModemStatus;
		if (!GetCommModemStatus(hComm, &dwModemStatus)) {
			// Error in GetCommModemStatus;
			ofLogError("ofSerial") << "Error getting the status of the CD line.";
		}
