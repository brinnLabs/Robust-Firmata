...
public:
	...
	/// to pin 5.
	AccelStepper(byte interface = AccelStepper::FULL4WIRE, byte pin1 = 2, byte pin2 = 3, byte pin3 = 4, byte pin4 = 5);
	...
	void		moveTo(int16_t absolute);

	/// Set the target position relative to the current position
	/// \param[in] relative The desired position relative to the current position. Negative is
	/// anticlockwise from the current position.
	void		move(int16_t relative);
	...
	void		runToNewPosition(int16_t position);
	...
	/// Sets the acceleration parameter.
	/// \param[in] acceleration The desired acceleration in steps per second
	/// per second. Must be > 0.0. This is an expensive call since it requires a square 
	/// root to be calculated. Dont call more ofthen than needed
	void		setAcceleration(uint16_t acceleration);

	/// Sets the deceleration parameter.
	/// \param[in] deceleration The desired deceleration in steps per second
	/// per second. Must be > 0.0. This is an expensive call since it requires a square 
	/// root to be calculated. Dont call more ofthen than needed
	...
	/// \sa setPinsInverted
	void		setEnablePin(byte enablePin = 0xff);
...
protected:
	...
	virtual void	setOutputPins(byte mask);
	...
	virtual void	step(byte step);

	/// Called to execute a step using stepper functions (pins = 0) Only called when a new step is
	/// required. Calls _forward() or _backward() to perform the step
	/// \param[in] step The current step phase number (0 to 7)
	virtual void	step0(byte step);
	...
	/// pin3, pin4.
	/// \param[in] step The current step phase number (0 to 7)
	virtual void	step8(byte step);

private:
	/// Number of pins on the stepper motor. Permits 2 or 4. 2 pins is a
	/// bipolar, and 4 pins is a unipolar.
	byte			_interface;          // 0, 1, 2, 4, 8, See MotorInterfaceType
	...
	int16_t		_currentPos;    // Steps

	/// The target position in steps. The AccelStepper library will move the
	/// motor from the _currentPos to the _targetPos, taking into account the
	/// max speed, acceleration and deceleration
	int16_t		_targetPos;     // Steps
	...
	float			_initialStepSize;

	/// Last step size in microseconds
	float			_prevStepSize;

	/// Min step size in microseconds based on maxSpeed
	float			_minStepSize; // at max speed
